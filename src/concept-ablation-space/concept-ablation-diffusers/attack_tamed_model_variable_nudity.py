...
MODEL_ROOT = '/home/shimon/research/concepts_erasure_models/checkpoints/Baselines/localscratch/damon2024/DM_baselines/ALL_baseline_ckpt/nudity/'

def parse_args():
    parser = argparse.ArgumentParser(description='Process some inputs.')
    parser.add_argument('--model_root', type=str, help='defense models root', default=MODEL_ROOT)
    parser.add_argument('--method', type=str, help='method name', choices=['EraseDiff', 'ESD', 'FMN', 'Salun', 'Scissorhands', 'SPM', 'UCE'])
    parser.add_argument('--prompt', type=str, help='Prompt for the model', default='nudity')
    parser.add_argument('--image_path', type=str, help='Nudity image to invert', default='')
    parser.add_argument('--target_root', type=str, help='Nudity image to invert', default='')
...
args = parse_args()
method_to_model_path = {'EraseDiff': osp.join(args.model_root, 'EraseDiff', 'EraseDiff-Nudity-Diffusers-UNet.pt'),
                        ...
                        'Scissorhands': osp.join(args.model_root, 'Scissorhands', 'Scissorhands-Nudity-Diffusers-UNet.pt'),
...
if args.image_path == '':
    ...
    image1 = Image.open(args.image_path)

image_path = osp.join(args.target_root, f"attack_input_{args.method}_image#1.png")
...
curr_image_name_ = (args.image_path).split('/')[-1].split('.png')[0]
RESULT_IMAGE_PATH = osp.join(args.target_root, f"attack_output_{args.method}_image_{curr_image_name_}.png")
...
new_out_image_path = RESULT_IMAGE_PATH.split('.png')[0]+'_2.png'
Image.fromarray(np.uint8(recon_img[0] * 255)).save(new_out_image_path)