...
# MODEL_ROOT = '/home/shimon/research/concepts_erasure_models/checkpoints/Baselines/object/localscratch/damon2024/DM_baselines/ALL_baseline_ckpt/object/'
# method_to_model_path = {'EraseDiff': osp.join(MODEL_ROOT, 'EraseDiff', 'EraseDiff-Parachute-Diffusers-UNet.pt'),
#                         'ESD': osp.join(MODEL_ROOT, 'ESD', 'ESD-Parachute-Diffusers-UNet-noxattn.pt'),
#                         'FMN': osp.join(MODEL_ROOT, 'FMN', 'FMN-Parachute-Diffusers-UNet.pt'),
#                         'Salun': osp.join(MODEL_ROOT, 'Salun', 'Salun-Parachute-Diffusers-UNet.pt'),
#                         'Scissorhands': osp.join(MODEL_ROOT, 'Scissorhands', 'Scissorhands-Parachute-Diffusers-UNet.pt'),
#                         'SPM': osp.join(MODEL_ROOT, 'SPM', 'SPM-Parachute-Diffusers-UNet.pt'),
#                         'UCE': osp.join(MODEL_ROOT, 'UCE', 'UCE-Parachute-Diffusers-UNet.pt'),
# }

def parse_args():
    parser = argparse.ArgumentParser(description='Process some inputs.')
    parser.add_argument('--method', type=str, help='method name')
    parser.add_argument('--prompt', type=str, help='Prompt for the model', default='Parachute')
...
args = parse_args()
MODEL_ROOT = '/home/shimon/research/concepts_erasure_models/checkpoints/Baselines/object/localscratch/damon2024/DM_baselines/ALL_baseline_ckpt/object/'
method_to_model_path = {'EraseDiff': osp.join(MODEL_ROOT, 'EraseDiff', f'EraseDiff-{args.prompt}-Diffusers-UNet.pt'),
                        'ESD': osp.join(MODEL_ROOT, 'ESD', f'ESD-{args.prompt}-Diffusers-UNet-noxattn.pt'),
                        'FMN': osp.join(MODEL_ROOT, 'FMN', f'FMN-{args.prompt}-Diffusers-UNet.pt'),
                        'Salun': osp.join(MODEL_ROOT, 'Salun', f'Salun-{args.prompt}-Diffusers-UNet.pt'),
                        'Scissorhands': osp.join(MODEL_ROOT, f'Scissorhands', 'Scissorhands-{args.prompt}-Diffusers-UNet.pt'),
                        'SPM': osp.join(MODEL_ROOT, 'SPM', f'SPM-{args.prompt}-Diffusers-UNet.pt'),
                        'UCE': osp.join(MODEL_ROOT, 'UCE', f'UCE-{args.prompt}-Diffusers-UNet.pt'),
}
...
image_path = f"attack_input_{args.method}_{prompt.replace(' ', '')}.png"
image1.save(image_path)

model_path = method_to_model_path[args.method]
# import ipdb; ipdb.set_trace()
pipe.unet.load_state_dict(torch.load(model_path))
pipe.to(device)
SRC_IMAGE = image_path
RESULT_IMAGE_PATH = f"attack_output_{args.method}_{prompt.replace(' ', '')}.png"
# SRC_IMAGE = 'MRI_of_Human_Brain.justpg'
# RESULT_IMAGE_PATH = 'demo_result_MRI_of_Human_Brain.png'

# SRC_IMAGE = 'unreal_image_blend.png'
# RESULT_IMAGE_PATH = 'demo_result_unreal_image_blend.png'
