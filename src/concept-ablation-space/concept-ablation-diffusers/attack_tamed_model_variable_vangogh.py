...
MODEL_ROOT = '/home/shimon/research/concepts_erasure_models/checkpoints/Baselines/vangogh/localscratch/damon2024/DM_baselines/ALL_baseline_ckpt/vangogh/'
method_to_model_path = {
                        'ESD': osp.join(MODEL_ROOT, 'ESD', 'ESD-VanGogh-Diffusers-UNet-noxattn.pt'),
                        'FMN': osp.join(MODEL_ROOT, 'FMN', 'FMN-VanGogh-Diffusers-UNet.pt'),
                        'AC': osp.join(MODEL_ROOT, 'AC', 'AC-VanGogh-Diffusers-UNet.pt'),
                        'SPM': osp.join(MODEL_ROOT, 'SPM', 'SPM-VanGogh-Diffusers-UNet.pt'),
                        'UCE': osp.join(MODEL_ROOT, 'UCE', 'UCE-VanGogh-Diffusers-UNet.pt'),
}

def parse_args():
    parser = argparse.ArgumentParser(description='Process some inputs.')
    parser.add_argument('--method', type=str, help='method name', choices=method_to_model_path.keys())
    parser.add_argument('--prompt', type=str, help='Prompt for the model', default='vangogh')
