...
def generate_images(
    ...
    """
    # model_base = "stabilityai/stable-diffusion-2-1-base"     #for SD 2.1
    # model_base = "stabilityai/stable-diffusion-2-base"       #for SD 2.0
    # model_base = "CompVis/stable-diffusion-v1-4"             #for SD 1.4

    # 1. Load the autoencoder model which will be used to decode the latents into image space.
    vae = AutoencoderKL.from_pretrained(model_base, subfolder="vae", cache_dir=cache_dir)
    # 2. Load the tokenizer and text encoder to tokenize and encode the text.
    tokenizer = CLIPTokenizer.from_pretrained(
        model_base, subfolder="tokenizer", cache_dir=cache_dir
    )
    text_encoder = CLIPTextModel.from_pretrained(
        model_base, subfolder="text_encoder", cache_dir=cache_dir
    )
    # 3. The UNet model for generating the latents.
    unet = UNet2DConditionModel.from_pretrained(
        model_base, subfolder="unet", cache_dir=cache_dir
    )
    ...
    for i, row in df.iterrows():
        ...
        guidance_scale = (
            row.sd_guidance_scale
            if hasattr(row, "sd_guidance_scale")
            else guidance_scale
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--ckpt", help="ckpt path", type=str, required=False, default=None
    )
